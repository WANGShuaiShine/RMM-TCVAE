...
class ModalityCommonDenoising(nn.Module):
    """模态公共去噪模块（MCD）
    
    该模块用于提取跨所有模态共享的去噪、模态不变特征。
    Conv1D、VIB和Attention层的参数是共享的。
    """
    def __init__(self, input_dim, hidden_dim, output_dim, num_heads=4, dropout=0.5):
        ...
        Args:
            ...
            dropout: Dropout概率
        """
        super(ModalityCommonDenoising, self).__init__()
        
        # 1D时间卷积层，用于标准化模态维度和序列长度
        # 注意：这里的参数是共享的，所有模态使用同一个卷积层
